...
<head>
    ...
    <link rel="stylesheet" href="freshmenenrollments.css">
    <title>Freshmen Enrollment Form</title>
</head>
<body>

    <div class="container mt-0">
          <a href="index.php"><i class="fa-sharp fa-solid fa-chevron-left"></i></a>
        <header>Enrollment Form</header> <span style="display: block; text-align: center;" id="check"></span>
        <form action="EnrollmentF-submit.php" method="post" enctype="multipart/form-data" id="enrollmentformF">
            <div class="form">
              <div class="fields">  
              ...
              <div class="input-field-2">
                      ...
                      <div class="input-field-2">
                        ...
                        <div class="input-field-2">
                            ...
                            <select  name="Yearlevel">
                                <option readonly selected>1st Year</option>
                            </select>
                        </div>
                      </div>
                </div>
                ...
                <div class="school require">
                    ...
                    <div class="buttons">
                        <button class="sumbit" type="submit" name="Esend" id="btns">
...
$(document).ready(function(){
    $('#Lastname, #Givenname, #Middlename, #Email').on('input', function(){
        var name1 = $('#Givenname').val() + ' ' + $('#Middlename').val() + ' ' + $('#Lastname').val();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(response == 'exists'){
                    ...
                    $('#Email').css('border-color', 'red').prop('required', true);
                    $('#btns').prop('disabled', true).css('background-color', '#999');
                } else if(response == 'name_exist') {
                    $('#check').html('Name already exists.').css('color', 'red');
                    $('#Lastname').css('border-color', 'red').prop('required', true);
                    $('#Middlename').css('border-color', 'red').prop('required', true);
                    $('#Givenname').css('border-color', 'red').prop('required', true);
                    $('#Email').css('border-color', '#007bff').prop('required', false);
                    $('#btns').prop('disabled', true).css('background-color', '#999');
                    ...
                    $('#Givenname').css('border-color', '#007bff').prop('required', false);
                    $('#Email').css('border-color', '#007bff').prop('required', false);
                    $('#btns').prop('disabled', false).css('background-color', '#007bff');
                }
            },
